...
(() => {
  ...
  let selectedIndex = 0;
  let handX = 0.5;
  let orbitOffsetX = 0;       // for hand-follow in EXPLODE
  ...
  const CONFIG = {
    ...
    photoOrbitRadius: 36,
    photoOrbitZ: 45,
    photoOrbitY: 8,          // raise orbit a bit higher on screen
    // Khi "Xem ảnh" (PHOTO) đưa ảnh lên giữa màn hình theo trục Y
    // (camera đang ở y=10 và nhìn về (0,0,0) nên y~8 sẽ gần trung tâm khung hình)
    photoViewY: 8,
    photoFollowRange: 34,    // how far orbit follows hand left/right
    photoFollowSmoothing: 0.12,
    
    // Camera beauty filter: trắng sáng + tone "hồng lạnh" ✨
    // Bạn có thể chỉnh nhẹ/tăng mạnh ở đây.
    cameraFilter: "brightness(1.22) contrast(1.08) saturate(1.18) hue-rotate(-8deg)",
    // 2 lớp tint: 1) hồng (screen) 2) xanh lạnh (screen) để ra "hồng lạnh"
    cameraTint: "rgba(255,105,180,0.14)",
    cameraTint2: "rgba(180,220,255,0.03)",
    cameraVignette: 0.12,                   // 0..0.25
    treeHeight: 70,
    treeBaseRadius: 34,
    snowCount: 1200,
    snowBox: 140
  ...
  function createCustomTexture(type) {
    ...
    function drawImageContain(img) {
      ...
      const iw = img.videoWidth || img.width || w;
      const ih = img.videoHeight || img.height || h;
      if (!iw || !ih) {
  ...
  function createDecorations() {
    ...
    lCtx.shadowBlur = 40;
    lCtx.fillText("cot đi ăn", 512, 138);
  ...
  function animate() {
    ...
    // Hand-follow for photo orbit (only when open hand / EXPLODE)
  ...
  async function initHands() {
    ...
    // Draw preview without distortion (fit/contain into 3:4 frame)
    function drawPreview(image) {
      ...
      // Beauty filter: trắng sáng + tone "hồng lạnh"
      ...
      // Soft glow overlays (screen): tạo cảm giác "trắng sáng" + tone "hồng lạnh"
      // 1) pink tint
      ...
      // 2) cool tint (optional) để "hồng" nhưng vẫn "lạnh"
      ...
      // Subtle vignette for a nicer look
  ...
  async function startSystem() {
    ...
    try {
      await initHands();
      if (imgStatus) imgStatus.textContent = "Sẵn sàng ✨ Mở tay để nổ, đưa tay trái/phải để kéo vòng ảnh, chụm ngón để xem ảnh.";
    } catch (e) {
