...
(() => {
  ...
  // Custom (user picked) URLs (for revoke)
  const customPhotoUrls = new Array(5).fill(null);
  ...
  function loadInitialPhotos() {
    const hasCustom = customPhotoUrls.some(Boolean);
    for (let i = 0; i < 5; i++) {
      const url = (hasCustom && customPhotoUrls[i]) ? customPhotoUrls[i] : defaultPhotoFiles[i];
      loadPhotoByUrl(i, url);
    }
  }

  function clearCustomPhotos() {
    for (let i = 0; i < 5; i++) {
      if (customPhotoUrls[i]) {
        URL.revokeObjectURL(customPhotoUrls[i]);
        customPhotoUrls[i] = null;
      }
    }
  }
  ...
  function animate() {
    ...
    if (state === "HEART") {
      ...
      for (let i = 0; i < photoMeshes.length; i++) {
        ...
        if (z > 6) {
          const ds = 1.05 + (z / CONFIG.photoOrbitRadius) * 0.80;
      ...
      for (let i = 0; i < photoMeshes.length; i++) {
        ...
        if (i === selectedIndex) {
          mesh.position.lerp(new THREE.Vector3(0, 0, 62), 0.12);
          mesh.scale.lerp(new THREE.Vector3(5.2 * aspect, 5.2, 5.2), 0.12);
          mesh.lookAt(camera.position);
          mesh.rotation.z = 0;
        } else {
  ...
  async function startSystem() {
    ...
    imgStatus.textContent = "Đang khởi động… Hãy cho phép Camera nếu được hỏi.";
    ...
    try {
      await initHands();
      imgStatus.textContent = "Sẵn sàng ✨ Mở tay để nổ, chụm ngón để xem ảnh.";
    } catch (e) {
      imgStatus.textContent = "Không mở được Camera. Bạn vẫn có thể xem hiệu ứng 3D và đổi ảnh.";
      console.warn(e);
    }
  }

  function handleImagePick(files) {
    const arr = Array.from(files || []).slice(0, 5);
    if (!arr.length) return;

    // clear old custom urls first
    clearCustomPhotos();

    for (let i = 0; i < 5; i++) {
      if (!arr[i]) continue;
      const url = URL.createObjectURL(arr[i]);
      customPhotoUrls[i] = url;
      loadPhotoByUrl(i, url);
    }

    imgStatus.textContent = `Đã chọn ${arr.length} ảnh. Dùng tay mở để xem ảnh bay ✨`;
  }

  function resetToDefault() {
    clearCustomPhotos();
    loadDefaultPhotos();
    imgStatus.textContent = "Đã trở về ảnh mặc định.";
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Keyboard accessibility for the label button
    const label = imgPicker.closest("label");
    if (label) {
